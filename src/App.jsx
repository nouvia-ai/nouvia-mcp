  ...
  return (
    <div>
      ...
      {[...notes].reverse().map((n, i) => (
        <div key={i} className="mb-3 p-3 bg-zinc-800/40 rounded-lg border border-zinc-800">
  ...
  return (
    <div>
      ...
      <div className="space-y-2">
        {clients.map(c => (
          <button key={c.id} onClick={() => setSelected(c)} className="w-full text-left p-3 bg-zinc-800/40 hover:bg-zinc-800 rounded-lg border border-zinc-800 hover:border-zinc-700 transition-all">
            <div className="flex items-center justify-between">
              <div><span className="text-sm font-medium text-zinc-200">{c.name}</span><span className="text-xs text-zinc-600 ml-2">{c.industry}</span></div>
              <div className="flex items-center gap-2"><Badge variant={stageBV(c.adoptionStage)}>{c.adoptionStage}</Badge><span className="text-xs text-zinc-600">{(c.entries||[]).length} notes</span></div>
            </div>
          </button>
        ))}
      </div>
    ...
    <div>
      ...
      <div className="space-y-2">
        {[...experiments].reverse().map(exp => (
          <div key={exp.id} className="p-3 bg-zinc-800/40 rounded-lg border border-zinc-800">
    ...
    <div className={`bg-zinc-900/80 border ${BLOCK_COLORS[block.id]} rounded-lg p-2.5 flex flex-col min-h-0 overflow-hidden`} style={GRID_LAYOUT[block.id]}>
      ...
      {adding ? (
        <div className="mt-1.5 flex-shrink-0">
          <input className="w-full bg-zinc-800 border border-zinc-700 rounded px-2 py-1 text-xs text-zinc-100 placeholder-zinc-600 focus:outline-none focus:border-zinc-500"
...
function CanvasTab({ canvas, setCanvas, saveCanvas }) {
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <div>
          <h2 className="text-sm font-semibold text-zinc-300 uppercase tracking-wider">Business Model Canvas</h2>
